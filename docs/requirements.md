...
### 1.1
The project shall provide a lean Arduino library for a 20x4 HD44780-compatible character LCD accessed through a PCF8574-based I2C backpack.
...
### 1.3
The library shall exclude non-essential device features from v1 unless they are required to support the primary line-oriented display use case.
...
### 2.3
All logic that does not inherently require Arduino headers or hardware access shall be isolated in host-testable C++.
...
### 2.5
Backpack-to-LCD pin mapping shall be treated as configuration, not as a fixed universal standard.

### 2.6
Backpack configuration shall support at minimum:
...
### 3.2
The library shall rely on the LCD controller's built-in character generator for ordinary character display.
...
### 3.4
Supporting the full printable 7-bit ASCII range is acceptable and preferred if it is straightforward.

### 3.5
Custom glyph definition support shall not be required for v1.

### 3.6
Physical 180-degree display rotation shall not be treated as a software feature requirement for v1.
...
### 4.4
A line write shorter than 20 characters shall be padded on the right with spaces to a full 20-character logical line.
...
### 4.10
The library shall provide binary backlight control equivalent to:

- `backlightOn()`
- `backlightOff()`

### 4.11
The library may expose additional low-level or convenience APIs, but the line-oriented API shall remain the primary supported interface.
...
### 5.6
Turning the display back on shall cause the visible display to match the most recent cached content.
...
### 6.3
The library shall support binary backlight on/off.

### 6.4
Scrolling features shall not be required for v1.

### 6.5
Other advanced LCD features shall remain out of scope for v1 unless needed to support the required line-oriented behavior.
...
### 8.4
The project shall include black-box tests for public API behavior where possible on host side.
...
### 9.3
The hardware smoke sketch shall be suitable for iterative extension during development.
...
### 10.3
Bottom-up implementation of dependencies is acceptable and expected where an API depends on lower-level behavior.
